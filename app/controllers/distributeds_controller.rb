class DistributedsController < ApplicationController
  ...
  if current_user.admin?
    ...
    for i in 1..104 do
      ...
      myshowcount = Performance.showing(mystart, (mystart+7)).uniq.pluck(:id).count

      myinfraredcount = Distributed.datespan(mystart, (mystart+7)).infrared.uniq.pluck(:performance_id).count
      myspecialservicescount = Distributed.datespan(mystart, (mystart+7)).translation.uniq.pluck(:performance_id).count
      myrepcount = Distributed.datespan(mystart, (mystart+7)).infrared.where("representative > 1").count

      if myinfraredcount == 0 && myspecialservicescount == 0 then
        mybuttonclass = "btn btn-medium btn-danger"
      else
        if myinfraredcount < myshowcount then
          mybuttonclass = "btn btn-medium btn-warning"
        else
          mybuttonclass = "btn btn-medium btn-primary"
        end
      end
